...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  final HomeRepository _homeRepository = const MockHomeRepository();
  int _homeCartItemTypes = 0;
  int _localAddedCartItemTypes = 0;
  ...
  void initState() {
    ...
    _loadHomeCartItemTypes();
  }

  Future<void> _loadHomeCartItemTypes() async {
    try {
      final data = await _homeRepository.fetchInitialData();
      if (!mounted) {
        return;
      }
      setState(() {
        _homeCartItemTypes = data.cartItemTypes;
      });
    } catch (_) {
      // Keep fallback value when initial home data is unavailable.
    }
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => VariationBottomSheet(
        ...
        onConfirm: (size, color, quantity, voucher) {
          ...
          final hadTypeBefore = cartProvider.items.any(
            (item) =>
  ...
  Widget build(BuildContext context) {
    ...
    final providerCartItemTypes = cartProvider.items.length;
    final syncedCartCount = [
      _homeCartItemTypes + _localAddedCartItemTypes,
      providerCartItemTypes,
    ].reduce((a, b) => a > b ? a : b);
...
class _RelatedProductsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final listHeight = textScale > 1.15 ? 222.0 : 210.0;
